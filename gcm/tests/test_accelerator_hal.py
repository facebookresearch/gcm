...
class _UnhealthyBackend(_FakeBackend):
    def __init__(self, backend_name: BackendName = BackendName.ROCM_SMI) -> None:
        self._backend_name = backend_name
        self.closed = False
...
def test_manager_probe_all_mixed_backend_states() -> None:
    healthy = _HealthyBackend(BackendName.NVML)
    unhealthy = _UnhealthyBackend(BackendName.ROCM_SMI)
    manager = AcceleratorManager(
        factories={
            BackendName.NVML: lambda: healthy,
            BackendName.ROCM_SMI: lambda: unhealthy,
        }
    )
    results = manager.probe_all()

    assert results[BackendName.NVML].healthy is True
    assert results[BackendName.ROCM_SMI].healthy is False
    assert manager.get_backend(BackendName.NVML) is healthy
    assert manager.get_backend(BackendName.ROCM_SMI) is None
